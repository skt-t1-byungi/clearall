...
type EventName<O extends Listenable> = O extends Window
    ...
    : O extends XMLHttpRequest
    ? keyof XMLHttpRequestEventMap
    : O extends { addEventListener: AnyFn }
    ? Parameters<O['addEventListener']>[0]
    : O extends { addListener: AnyFn }
    ? Parameters<O['addListener']>[0]
    : O extends { on: AnyFn }
    ? Parameters<O['on']>[0]
    : O extends { subscribe: AnyFn }
    ? Parameters<O['subscribe']>[0]
    : string

type ListenerBy<O extends Listenable, K extends string> = O extends Window
    ? K extends keyof WindowEventMap
        ? Fn<[WindowEventMap[K]]>
        : never
    : O extends Document
    ? K extends keyof DocumentEventMap
        ? Fn<[DocumentEventMap[K]]>
        : never
    : O extends HTMLElement
    ? K extends keyof HTMLElementEventMap
        ? Fn<[HTMLElementEventMap[K]]>
        : never
    : O extends WebSocket
    ? K extends keyof WebSocketEventMap
        ? Fn<[WebSocketEventMap[K]]>
        : never
    : O extends XMLHttpRequest
    ? K extends keyof XMLHttpRequestEventMap
        ? Fn<[XMLHttpRequestEventMap[K]]>
        : never
    : O extends { addEventListener: Listener<K, infer Args> }
    ? Fn<Args>
    : O extends { addListener: Listener<K, infer Args> }
    ? Fn<Args>
    : O extends { on: Listener<K, infer Args> }
    ? Fn<Args>
    : O extends { subscribe: Listener<K, infer Args> }
    ? Fn<Args>
    : AnyFn

type Adder = <O extends Listenable, K extends EventName<O>>(
    o: O,
    name: K,
    listener: ListenerBy<O, K>,
    ...params: any[]
) => ClearAll
type ClearAll = AnyFn & { add: Adder }

export function add(): ClearAll
export function add<O extends Listenable, K extends EventName<O>>(
    o: O,
    name: K,
    listener: ListenerBy<O, K>,
    ...params: any[]
...
export function add<O extends Listenable, K extends EventName<O>>(
    ...
    clearAll.add = <O extends Listenable, K extends EventName<O>>(
        o: O,
        name: K,
        listener: ListenerBy<O, K>,
        ...params: any[]
